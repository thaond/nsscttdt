...
public class PartnerFinderImpl extends BasePersistenceImpl implements PartnerFinder {
	
	public static String FIND_BY_TEN_MOTA = PartnerFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = PartnerFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_partner)", StringPool.LIKE, false, names);
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(mo_ta_partner)", StringPool.LIKE, true, des);
	...
	public List<Partner> findTEN_MOTA(String name, String des, boolean andOperator, 
						int start, int end, OrderByComparator obc) throws SystemException {
		
		return findTEN_MOTA(new String[]{name}, new String[]{des}, andOperator, start, end, obc);
	}

	public List<Partner> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_partner", PartnerImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<Partner>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
