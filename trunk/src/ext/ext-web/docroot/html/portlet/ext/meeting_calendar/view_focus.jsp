...
<script type="text/javascript">
	function <portlet:namespace />addFocus() {
		var url = '<liferay-portlet:renderURL windowState="<%= WindowState.NORMAL.toString() %>"><portlet:param name="tabs1" value="focus-week" /><portlet:param name="struts_action" value="/ext/meeting_calendar/edit_focus_week" /><portlet:param name="week" value="<%= String.valueOf(week)%>" /><portlet:param name="year" value="<%= String.valueOf(year)%>" /><portlet:param name="redirect" value="<%= currentURL %>" /></liferay-portlet:renderURL>';		
	...
	function <portlet:namespace />editFocus(mdweekId) {
		...
		var url = '<liferay-portlet:renderURL windowState="<%= WindowState.NORMAL.toString() %>"><portlet:param name="tabs1" value="focus-week" /><portlet:param name="struts_action" value="/ext/meeting_calendar/edit_focus_week" /><portlet:param name="week" value="<%= String.valueOf(week)%>" /><portlet:param name="year" value="<%= String.valueOf(year)%>" /><portlet:param name="mdweekId" value="<%= String.valueOf(mdweekId)%>" /><portlet:param name="<%= Constants.CMD%>" value="<%= Constants.EDIT%>" /><portlet:param name="redirect" value="<%= currentURL %>" /></liferay-portlet:renderURL>';		
...
<tr>
	<td align="right">
		...
		<tr>
			<td style="font-size:12px; text-align: center;" colspan="3">
				...
				<img border="0" src="/html/portlet/ext/meeting_calendar/Prev_Button.gif" />
				</a>&nbsp;&nbsp;
				...
				&nbsp;&nbsp;
				<a href="<%= portletURL.toString() %>&<portlet:namespace />month=<%= cal.get(Calendar.MONTH) %>&<portlet:namespace />day=<%= cal.get(Calendar.DATE) %>&<portlet:namespace />year=<%= cal.get(Calendar.YEAR) %>">
				<img border="0" src="/html/portlet/ext/meeting_calendar/Next_Button.gif" />
				</a>
			</td>
		</tr>
		</table>
...
<tr>
	<td valign="top">
		<table class="lfr-table">
			<tr>
				<td class="focusWeek" valign="top" width="10%"><liferay-ui:message key ="trong-tam"/> :</td>
				<td>
				<%= focus %>			
				</td>
			</tr>
			...
			<tr>
				<td class="focusWeek" valign="top"><liferay-ui:message key ="luu-y"/> :</td>
				<td><%= note %>
				</td>
			</tr>
			<tr height="10px">
			...
			<tr>
				<td class="focusWeek" valign="top"><liferay-ui:message key ="noi-nhan"/> :</td>
				<td><%= receive%>
				</td>								
			</tr>	
		</table>
