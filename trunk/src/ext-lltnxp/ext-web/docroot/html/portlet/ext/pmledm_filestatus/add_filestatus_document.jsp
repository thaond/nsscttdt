...
<%
	String tabsAdd = request.getParameter("tabs");
	String redirectAdd = request.getParameter("redirect");
...
<portlet:actionURL var="add">
	<portlet:param name="struts_action" value="/sgs/pmledm_filestatus/add_filestatus_document"/>
	<portlet:param name="tabs" value="<%= tabsAdd %>"/>
</portlet:actionURL>
...
<div class="title_categ"><liferay-ui:message key="add_new"/></div>
	...
	<input name="<portlet:namespace />redirect" type="hidden" value="<%= HtmlUtil.escape(redirectAdd) %>" />
	...
	<table cellpadding="0" width="100%" >
		...
		<tr>
			...
			<td>
				<input class="input-pml" type="text" name="<portlet:namespace/>pml_edm_file_status_code" 
		...
		<tr>
			<td></td>
				<td>
					...
					<span onclick="javascript:submitForm(document.hrefFm,'<%= redirectAdd %>');"><input type="button"	value='<liferay-ui:message key="back"/>' /></span>
			 	</td>
			</tr>
	</table>
</div>
