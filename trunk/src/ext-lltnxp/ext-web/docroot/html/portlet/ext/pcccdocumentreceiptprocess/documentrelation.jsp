...
<%
	...
	portletURL.setParameter("struts_action", "/sgs/pcccdocumentreceiptprocess/documentrelation");
	...
	try {
		PmlEdmDocumentReceipt tmpDocRec = PmlEdmDocumentReceiptUtil.findByPrimaryKey(documentId);
		linkDesc += LanguageUtil.get(pageContext, "link-for-document-receipt") + ". ";
		linkDesc += LanguageUtil.get(pageContext, "pccc-cvdtn-sohieu") + ": " + tmpDocRec.getDocumentReference() + ". ";
		linkDesc += LanguageUtil.get(pageContext, "pccc-cvdtn-soCVden") + ": " + tmpDocRec.getNumberDocumentReceipt() + ". ";
		linkDesc += LanguageUtil.get(pageContext, "pccc-cvdtn-trichyeu") + ": " + tmpDocRec.getBriefContent() + ". ";
	}
	catch (Exception ex) {
		System.out.println("Khong doc duoc van ban den so:" + String.valueOf(documentId));
	}
...
<form action="<%= portletURL.toString() %>" enctype="multipart/form-data" method="post" name="<portlet:namespace />fm">
	...
	<label class="laborder"><liferay-ui:message key="link-document-exist"/></label>
		...
		List<PmlEdmDocumentRelation> relDocLefts = 
			PmlEdmDocumentRelationUtil.findByDocLeftId_LeftIsReceipt(documentId, true);
		...
		List<PmlEdmDocumentRelation> relDocRights = 
			PmlEdmDocumentRelationUtil.findByDocRightId_RightIsReceipt(documentId, true);
	...
	<label class="laborder"><liferay-ui:message key="create-link-document-new"/></label>
			...
			<c:choose>
				<c:when test='<%= tabnew.equals("cvden") %>'>
					...
					// khong cho lien ket voi chinh no
					if (resultReceipts.contains(documentId)) {
						resultReceipts.remove(documentId);
