...
<%
	...
	portletURLSend.setParameter("struts_action", "/sgs/pcccdocumentreceiptprocess/search");
...
<%
	...
	//xuancong close dua vao trong java headerNames.add(6, "nguoi-ky");
...
<liferay-ui:search-form
	page="/html/portlet/ext/pcccdocumentreceiptprocess/timcongvandi_filter.jsp"
	searchContainer="<%= searchContainerSend %>" />

<%
//try {// yenlt close 25052010
	
	if (Boolean.valueOf(renderRequest.getAttribute("sendFlag").toString()) == true) {
		...
		//xuancong close System.out.println(user.getCompanyId() + ",SoPhatHanh: " +  displayTerms.getSoPhatHanh() + ",LoaiCongVan: " +  displayTerms.getLoaiCongVan() + ",TraLoiCongVanSo: " +  displayTerms.getTraLoiCongVanSo() + ",PhongSoanThao: " +  displayTerms.getPhongSoanThao() + ",NoiNhan: " +  displayTerms.getNoiNhan() + ",NguoiKy: " +  displayTerms.getNguoiKy() + ", " +  fromDate + ", " +  toDate + ",SoHSCV: " +  displayTerms.getSoHSCV() + ",TrichYeu " +  displayTerms.getTrichYeu() + ", " +  searchContainerSend.getStart() + ", " +  searchContainerSend.getEnd());
		int total = 0;
		...
		//thoi start
		// Bo cot delete trong trang nay
		searchContainerSend.getHeaderNames().remove(10);
		...
		if(!PortletPermissionUtil.contains(permissionChecker, plid, "SGS_PCCCDOCUMENTRECEIPTPROCESS_INSTANCE_HKH8", ActionKeysExt.TIEPNHANCV)) {
			//xuancong close, cot edit khong con o vi tri thu 2 searchContainerSend.getHeaderNames().remove(1);
		...
		for (int i = 0; i < hits.getDocs().length; i ++) {
			...
			if ( null!= pmlEdmDocumentSend) { // end yenlt 25052010
				...
				for (int j = 0; j < pmlEdmAttachedFileList.size(); j++ ) {
					//xuancong close toanVan += "<a href='"+ pmlEdmAttachedFileList.get(j).getPath() +"'><u><img src='/html/images/icon_download.jpg'>&nbsp;" + pmlEdmAttachedFileList.get(j).getAttachedFileName().substring(14, pmlEdmAttachedFileList.get(j).getAttachedFileName().length()) + "</u></a> <br/>";
