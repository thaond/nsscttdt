...
<%@page import="com.sgs.portlet.onedoorpccc.service.PmlFileReturningInfoLocalServiceUtil"%>
...
<form action="<portlet:actionURL windowState="<%= WindowState.NORMAL.toString() %>">
	<portlet:param name="struts_action" value="/sgs/onedoorsearch/detail" /></portlet:actionURL>" enctype="multipart/form-data" method="post" name="<portlet:namespace />fm_View">

<div class="title_categ"><liferay-ui:message key="thong-tin-chi-tiet"/></div>
...
<ul>
	<li style="width: 49.6%; margin-right: 0.8%" >
		...
		<legend class="laborder"><liferay-ui:message key="pccc-TN-thongtinhoso"/></legend>
			...
			<tr>
				<td class="td-left" width="18%"><liferay-ui:message key="pccc-DSHS-mahoso"/>:</td>
	...
	<li style="width: 49.6%;">
		...
		<legend class="laborder"><liferay-ui:message key="pccc-TN-thongtinquanly"/></legend>
			...
			<tr valign="top">
				...
				<td class="td-right"><textarea class="textarea-display" readonly="" ><%=filePcccDTO.getNoteInformationManagement() %></textarea></td>
			</tr>
			</table>
		</fieldset>
		...
		<legend class="laborder"><liferay-ui:message key="pccc-TN-thongtintrahoso"/></legend>
			...
			<tr valign="top">
				<td class="td-left"><liferay-ui:message key="pccc-TN-ghichu"/>:</td>
				<td class="td-right"><textarea class="textarea-display" onMouseover="this.style.backgroundColor='white';" readonly="" rows="3" cols="45"><%=fileReturningInfo != null ? fileReturningInfo.getNote() : "" %></textarea></td>
			</tr>
			</table>
		</fieldset>
		<fieldset class="filborder">
		<legend class="laborder"><liferay-ui:message key="pccc-TN-ketluanvehosothamduyet"/></legend>
			...
			<tr valign="top">
				...
				<td class="td-right"><textarea class="textarea-display" readonly="" rows="3" cols="45"><%=filePcccDTO.getConclusion() %></textarea></td>
			</tr>
			</table>
		</fieldset>
...
<ul>
	<li style="width: 100%">
		...
		<legend class="laborder"><liferay-ui:message key="pccc-TN-hosodinhkem"/></legend>
				...
				<table class="taglib-search-iterator table-pml" cellspacing="0" width="100%" style="display: none">	
					<tr class="portlet-section-header results-header" >
						<td width="35px" align="center"><liferay-ui:message key="pccc-TN-stt"/></td>
			...
			<table class="taglib-search-iterator table-pml" cellspacing="0" width="100%">	
					...
					<logic:iterate id="processDispose" name="processDisposeList" type="com.sgs.portlet.document.model.PmlProcessDispose" indexId="k">
						<tr class="${((k % 2)==0)? 'results-row':'tr_two results-row'}">
							...
							%>
							<td width="20%"><%=processDispose.getProcesser() != 0 ? processerName : ""%></td>
							...
							<td width="20%"><%=processDispose.getReceiver() != 0 ? nameReceive : ""%></td>
							...
							<td width="25%"><%=processDispose.getProcessInfomation().replace("/\\/", "<br />") %></td>
						</tr>
					</logic:iterate>
				</table>
		</fieldset>
...
<div id ="table-button" align="right">
	...
	%>	
		<portlet:actionURL windowState="<%= WindowState.NORMAL.toString() %>" var="back">
			<portlet:param name="struts_action"	value="/sgs/onedoorsearch/view" />
		</portlet:actionURL> 
