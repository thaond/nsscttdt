...
import com.sgs.portlet.document.service.PmlProcessDisposeLocalServiceUtil;
...
import com.sgs.portlet.onedoorpcccprocess.util.InBienNhanUtil;
...
public class OneDoorProcessLiferayWorkflowService extends
	...
	private static Log _log = LogFactory.getLog(OneDoorProcessLiferayWorkflowService.class);
	private final static String transitionSeperator = "%";
	private final static String nodeSeperator = ":";
	private final static Logger log = Logger
			.getLogger(OneDoorProcessLiferayWorkflowService.class);
	...
	public void saveDataIntiPmlTransition(String fileTypeId, String nameProcess, int version, int traceTransitionProcessMain) {
		...
		try {
			for (String[] nameString : nameTransitionList) {			
						...
						List<PmlStateProcess> pmlStateProcessCurrentList = PmlStateProcessUtil.findByStateProcessCode(stateNodeCurrentCode.concat(TYPE_MC)); 
						...
						List<PmlStateProcess> pmlStateProcessNextList = PmlStateProcessUtil.findByStateProcessCode(stateNodeNextCode, TYPE_MC); 
						...
						pmlTransition.setNoTransition(PmlTransitionUtil.countByFileTypeId(fileTypeId) + 1);
	...
	public void deloyProcessSupport(String fileTypeId,InputStream is, String nameProcess,int traceTransitionProcessMain) {
		...
		try {
			pmlStateProcessList = PmlStateProcessUtil.findByStateProcessName(TYPE_MC);
		...
		for (String nodeName : nodeNameList) {
			...
			if (!flag) {
				...
				try {
					...
					log.error("ERROR: no upadate PmlStateProcess in method deloyProcessSupport " + DocumentReceiptLiferayWorkflowService.class);
				}
			}
		...
		if (nodeList != null ) {
			...
			try {
				pmlWorkflowList = PmlWorkflowUtil.findByWorkFlow_Version(nameProcess, version);
				...
				for (String nodeName : nodeList) {	
					...
					if(pmlWorkflowList != null && pmlWorkflowList.size() > 0){
						...
						PmlStateWorkFlow pmlStateWorkFlow = new PmlStateWorkFlowImpl();
						 ...
						 pmlStateProcessList = PmlStateProcessUtil.findByStateProcessCode(arr[0], TYPE_MC);// yenlt update 10082010
...
//end 06/04/2010
	...
	public void addWorkflowInstance(List<WorkflowParam> params) {
		...
		ProcessInstance pi = getJbpmService().createProcessInstanceHasVersion(
	...
	private PmlStateProcess getStateProcessFromCode(String stateProcessCode) {
		...
		try {
			list = PmlStateProcessUtil.findByStateProcessCode(stateProcessCode, TYPE_MC);
		} catch (SystemException e1) {
...
//		return stateProcess;
	...
	protected ProcessDefinition getProcessDefinition(String fileTypeId, Date date) {
		...
		try {
			...
			List<PmlWorkflow> list = PmlWorkflowUtil.findByFileTypeId(fileTypeId);
			...
			pmlWorkflow = PmlWorkflowUtil.findByFileTypeId_Version(fileTypeId, wf.getVersion_()).get(0);
	...
	public ProcessDefinition getProcessDefinition(String fileId) {
		...
		try {
			pmlOneDoorReceiveFile = PmlOneDoorReceiveFileUtil
		...
		if ( null != file) {
			...
			fileTypeId = file.getFileTypeId();
			 try {
				...
				_log.error(e);
				_log.error("CAN NOT GET FILE TYPE");
			}
...
//				processDispose.setProcesser(userId); // yenlt close 24052010
	...
	public List<PmlStateProcess> getStateOfFileWithUser(long userId) {
		...
		for (PmlOneDoorReceiveFile pf : pmlOneDoorReceiveFileList) {
			...
			try {
				listTemp = PmlStateProcessUtil.findByStateProcessCode(nodeName, TYPE_MC);
			} catch (SystemException e) {}
