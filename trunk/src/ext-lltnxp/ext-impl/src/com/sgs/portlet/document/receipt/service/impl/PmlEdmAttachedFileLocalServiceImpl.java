 ...
 * @author XuanCong
 */
public class PmlEdmAttachedFileLocalServiceImpl extends
		PmlEdmAttachedFileLocalServiceBaseImpl {
	...
	public List<PmlEdmAttachedFile> getAttachedFileForLog(Class clazz, long id) {
		...
		try {
			retValue = PmlEdmAttachedFileUtil.findByObjectContentId_ObjectType(
					id, objectType);
	 ...
	 *            ma so van ban di hoac den
	 * @param objectType
	 *            loai van ban di hoac den
	 * @return
	 */
	public int deletePmlEdmAttachedFileByOO(long objectContentId,
		...
		try {
			...
			System.out
					.println("Co loi khi xoa file dinh kem theo ma so va loai.");
			ex.printStackTrace();
		} // end catch
		return retVal;
	}
	...
	public List<PmlEdmAttachedFile> findByObjectContentId_ObjectType(
		...
		List<PmlEdmAttachedFile> results = null;
		results = pmlEdmAttachedFilePersistence
				.findByObjectContentId_ObjectType(objectContentId, objectType);

		if (Validator.isNull(results)) {
			results = new ArrayList<PmlEdmAttachedFile>();
		}
