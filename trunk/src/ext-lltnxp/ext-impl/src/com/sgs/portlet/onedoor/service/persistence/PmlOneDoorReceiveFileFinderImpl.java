...
//	end minh update 20100525
	 ...
	 * pml_processdispose.receivedate IS NOT NULL 
	 * @throws SystemException 
	 */
	public List<PmlOneDoorReceiveFile> getListFileDaNhanChuaHoanThanh(long userId, List<String> typeList, List<String> valueTypeList, int start, int end,OrderByComparator obc) throws SystemException {
		...
		try {
			...
			sql += " FROM pml_processdispose ";
			sql += " WHERE pml_processdispose.processer = "+userId;
			sql += " AND pml_processdispose.receivedate  IS NOT NULL ";
			sql += " AND pml_processdispose.dateprocess IS NULL ";
			sql += " GROUP BY pml_processdispose.fileid ";
			sql += " ) AS pml_processdispose )  ";
			sql += " AND pml_file.exactreturningdate IS NULL ";
			sql += " ) ";
			sql += " AS pml_file, pml_wf_onedoor, pml_file_pccc ";
...
//		userIds.add(userId);
		...
		try {
			session = openSession();
//			OneDoorProcessLiferayWorkflowService workflowService = new OneDoorProcessLiferayWorkflowService();
//			List<Long> listProcessId = workflowService.getProcessInstances(userIds);
			
			String sql = " SELECT DISTINCT pml_file.* ";
			sql += "FROM pml_file , pml_wf_onedoor wfonedoor";	
//			sql += " FROM pml_file , pml_wf_onedoor wfonedoor, pml_file_pccc";
					//" pml_filetype ft ";
			sql += " WHERE pml_file.fileid "; 
			sql += " IN ( SELECT pml_processdispose.fileid "; 
			...
			sql += " )";  
			sql += " AND pml_file.exactreturningdate IS NULL "; 
			//sql += " AND pml_file.filetypeid = ft.filetypeid ";
...
//					sql = sql.replace( " FROM pml_file, pml_wf_onedoor ", " FROM pml_file, pml_wf_onedoor, pml_file_pccc");
	...
	//YENLT FILE CAN XU LY  ### FILE UY QUYEN
	/*
...
//			sql += " FROM pml_file, pml_wf_onedoor, pml_file_pccc ";
...
//					sql = sql.replace( " FROM pml_file, pml_wf_onedoor ", " FROM pml_file, pml_wf_onedoor, pml_file_pccc");
	...
	/**
	 * THU LY HO SO CAP PHONG - THU LY HO SO CAP SO - YEN - 24-05-2010
	 */
	// TON DAU KY
	...
	/**
	 * BAO CAO HO SO THANG LUY KE -- 24/05/2010 -- YEN
	 */
	...
	// end yenlt 24-05-2010
	
	/**
	 * yenlt create 16/10/2010
	...
	/**
	 * YENLT 16/10/2010 ONEDOORSEARCH
	 */
	public List<PmlOneDoorReceiveFile> onedoorSearch(String numberreceipt, String filename, String applicantname,
		...
		try {
			...
			if (!"".equals(tuNgayTiepNhan)) {
				...
				tuNgayTiepNhan = dateFormatTwo.format(dateFormatOne.parse(tuNgayTiepNhan));
				if (flag) {
					sql += " AND (pml_file_pccc.inputdatadate >= '" + tuNgayTiepNhan + "') ";
				} else {
					sql = sql.replace(" FROM pml_file ", " FROM pml_file,  pml_file_pccc ");
					sql += " AND (pml_file_pccc.inputdatadate >= '" + tuNgayTiepNhan + "') ";
					flag = true;
			...
			if (!"".equals(denNgayTiepNhan)) {
				...
				denNgayTiepNhan = dateFormatTwo.format(dateFormatOne.parse(denNgayTiepNhan));
				if (flag) {
					sql += " AND (pml_file_pccc.inputdatadate <= '" + denNgayTiepNhan + "') ";
				} else {
					sql = sql.replace(" FROM pml_file ", " FROM pml_file,  pml_file_pccc ");
					sql += " AND (pml_file_pccc.inputdatadate <= '" + denNgayTiepNhan + "') ";
					flag = true;
	...
	public int countSoLuongHoSoTonTruoc(List<Long> userIds, String fromDate, String toDate) throws SystemException {
		...
		try {
			...
			String sql = " SELECT COUNT(pml_file.fileid) AS COUNT_VALUE ";
			sql += " FROM ( ";	
			sql += 		" SELECT pml_processdispose.fileid, MAX(pml_processdispose.transition_ ) as tran ";
			sql += 		" FROM pml_processdispose  ";
			sql +=      " WHERE ( pml_processdispose.processer in (" + userList + ")) ";
			...
			sql += " AND ( pml_pro.fileid = pml_file.fileid ) "; 
			sql += " AND ( pml_processdispose.receivedate IS NOT NULL" +
				   " AND pml_processdispose.receivedate < '" + fromDate + "')";
			sql += " AND (pml_processdispose.dateprocess IS NULL OR pml_processdispose.dateprocess >= '" + fromDate +"') ";
	...
	public int countSoLuongHoSoNhanTrongThang(List<Long> userIds, String fromDate, String toDate) throws SystemException {
		...
		try {
			...
			sql += " AND ( pml_pro.fileid = pml_file.fileid )"; 
			sql += " AND ((pml_processdispose.receivedate IS NOT NULL" +
				   " AND pml_processdispose.receivedate >= '" + fromDate + "' AND pml_processdispose.receivedate <= '" + toDate + "')"; 
			sql += " OR ( pml_processdispose.receivedate IS NULL AND pml_processdispose.senddate IS NOT NULL" + 
				   " AND pml_processdispose.senddate <= '" + toDate + "'))";
	...
	public int countSoLuongHoSoDaXuLy(List<Long> userIds, String fromDate, String toDate) throws SystemException {
		...
		try {
			...
			sql += " AND ( pml_pro.fileid = pml_file.fileid ) "; 
			sql += " AND pml_processdispose.dateprocess IS NOT NULL ";
			sql += " AND pml_processdispose.dateprocess >= '" + fromDate + "' AND pml_processdispose.dateprocess <= '" + toDate + "' ";
