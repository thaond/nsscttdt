<%
if(event.getGroupId() == portletGroupId.longValue()){
%>
	<c:if test="<%= PortletPermissionUtil.contains(permissionChecker, plid.longValue(),portletDisplay.getId(),MeetingCalendarKey.APPROVE_CALENDAR) && 
		((mcal.getState() == MeetingCalendarKey.CHO_DUYET) || (mcal.getState() == MeetingCalendarKey.HUY))%>">
		<portlet:actionURL windowState="<%=WindowState.NORMAL.toString()%>" var="approveURL">
			...
			<portlet:param name="mcalId" value="<%=String.valueOf(mcal.getMcalId())%>" />
		</portlet:actionURL>
		<%
	...
	<c:if test="<%= ((CalEventPermission.contains(permissionChecker, event, ActionKeys.UPDATE) && (mcal.getState() == MeetingCalendarKey.CHO_DUYET)) ||
		PortletPermissionUtil.contains(permissionChecker, plid.longValue(), portletDisplay.getId(), ActionKeys.UPDATE)) &&
		(mcal.getState() != MeetingCalendarKey.DOI)
	  	%>">
		<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" var="editURL">
			<portlet:param name="struts_action" value="/ext/meeting_calendar/edit_event" />
			<portlet:param name="redirect" value="<%=currentURL%>" />
			<portlet:param name="tabs1" value="register-calendar" />
			<portlet:param name="eventId" value="<%=String.valueOf(mcal.getMcalId())%>" />
		</portlet:renderURL>
		<liferay-ui:icon image="edit" url="<%=editURL%>" />
	</c:if>
	
	<c:if test="<%= PortletPermissionUtil.contains(permissionChecker, plid.longValue(), portletDisplay.getId(), ActionKeys.UPDATE) && 
		(mcal.getState() == MeetingCalendarKey.DA_DUYET) && (mcal.getState() != MeetingCalendarKey.HUY) && (mcal.getState() != MeetingCalendarKey.DOI)
	  	%>">
		<portlet:renderURL windowState="<%=WindowState.MAXIMIZED.toString()%>" var="moveURL">
			<portlet:param name="struts_action" value="/ext/meeting_calendar/edit_event" />
			<portlet:param name="redirect" value="<%=currentURL%>" />
			<portlet:param name="tabs1" value="register-calendar" />
			<portlet:param name="eventId" value="<%=String.valueOf(mcal.getMcalId())%>" />
	...
	<c:if test="<%=
		((CalEventPermission.contains(permissionChecker, event , ActionKeys.DELETE) && (mcal.getState() == MeetingCalendarKey.CHO_DUYET)) ||
