...
import com.sgs.portlet.onedoorpcccreport_baocaohosothangluyke.searchfiledetail.PmlOneDoorReceiveFileSearch;
...
public class FileDetailsAction extends PortletAction {
	...
	private List<PmlOneDoorReceiveFile> getResult(String action, String departmentId,
		...
		for(int i = 0; i < pmlDepartmentList.size(); i ++) {
			...
			try {
				pmlUserList = PmlUserUtil.findByDepartmentsId(pmlDepartmentList.get(i).getDepartmentsId());
			} catch (Exception e) {
	...
	private List<PmlFilePcccDTO> add2DTO(Collection<PmlOneDoorReceiveFile> pmlOneDoorReceiveFileList){
		...
		for (PmlOneDoorReceiveFile pmlOneDoorReceiveFile : pmlOneDoorReceiveFileList) {
			...
			try {
				file = PmlOneDoorReceiveFileUtil.findByPrimaryKey(pmlOneDoorReceiveFile.getFileId());
			} catch (NoSuchPmlOneDoorReceiveFileException e1) {
			...
			if (log != null) {
				...
				PmlFileStatusStateProcess fileStatusStateProcess = null;
				try {
					fileStatusStateProcess = PmlFileStatusStateProcessUtil
										.findByStateProcessId(log.getStateProcessIdBefore()).get(0);
				} catch (Exception e) {
					try {
						fileStatusStateProcess = PmlFileStatusStateProcessUtil.findByStateProcessId(log.getStateProcessIdAfter()).get(0);
					} catch (Exception e1) {
				...
				try {
					logList = PmlProcessDisposeUtil.findByFileId(pmlOneDoorReceiveFile.getFileId());
				} catch (Exception e) {
				...
				if ( logList.size() != 0) {
					...
					try {
						listTemp = PmlProcessDisposeUtil.findByFileId_Transition(pmlOneDoorReceiveFile.getFileId(), logList.size());
					} catch (Exception e) {
					}
					if (listTemp.size() > 0) {
						log =listTemp.get(0);
