...
public class LoaiVanBanFinderImpl extends BasePersistenceImpl implements LoaiVanBanFinder {
	public static String FIND_BY_TEN_MOTA = LoaiVanBanFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = LoaiVanBanFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_loai_van_ban)", StringPool.LIKE, false, names);
	...
	public List<LoaiVanBan> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_loai_van_ban", LoaiVanBanImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<LoaiVanBan>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
