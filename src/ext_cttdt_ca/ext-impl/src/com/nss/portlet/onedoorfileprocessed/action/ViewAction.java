...
import com.nss.portlet.onedoorfileprocessed.search.OneDoorFileProcessedSearch;
...
public class ViewAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
			...
			if (getForward(req) != null && !getForward(req).equals("")) {
				...
				portletURL.setParameter("struts_action", "/nss/onedoorpcccprocessed/view");
				...
				return mapping.findForward("portlet.nss.onedoorfileprocessed.view");
			}
	}
		...
		for (PmlOneDoorReceiveFile file : list) {
			...
			if (log != null) {
				...
				if (0 != log.getStateProcessIdAfter()) {
					status = PmlStateProcessLocalServiceUtil.getPmlStateProcess(log.getStateProcessIdAfter());
				} else {
					status = PmlStateProcessLocalServiceUtil.getPmlStateProcess(log.getStateProcessIdBefore());
				}
