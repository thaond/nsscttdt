		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j = 0; j < eventsDuyet.size(); j++) {
				...
				if(mcal!=null){
					sbbuoisang.append(" - ");
					// thoi gian hop
					if (event.isAllDay()) { // Ca ngay
						sbbuoisang.append(bSt + "C\u1ea3 ng\u00e0y" + bEn + " ");
					}
					else if (mcal.isMorning()) { // Buoi sang
						sbbuoisang.append(bSt + "S\u00e1ng" + bEn + " ");
					}
					else {
					...
					// noi dung
					sbbuoisang.append(event.getDescription().replaceAll("\\<.*?\\>", "").replace("&nbsp;", " ") + "; ");
					// dia diem
			...
			for (int j = 0; j < eventsDuyet_E.size(); j++) {
				...
				if(mcal!=null){
					sbbuoichieu.append(" - ");
					// thoi gian hop
					if (mcal.isAfternoon()) { // Buoi chieu
						sbbuoichieu.append(bSt + "Chi\u1ec1u" + bEn + " ");
					}
					else {
						if(event.isTimeZoneSensitive()){
