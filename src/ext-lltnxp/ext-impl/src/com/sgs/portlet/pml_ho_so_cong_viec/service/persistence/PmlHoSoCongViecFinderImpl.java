 ...
 * 
 */
public class PmlHoSoCongViecFinderImpl extends BasePersistenceImpl implements
		PmlHoSoCongViecFinder {

	public static String COUNT_BY_S_T_N_I_V_I_I_T_U_H = PmlHoSoCongViecFinder.class
			.getName() + ".countByS_T_N_I_V_I_I_T_U_H";

	public static String FIND_BY_S_T_N_I_V_I_I_T_U_H = PmlHoSoCongViecFinder.class
			.getName() + ".findByS_T_N_I_V_I_I_T_U_H";

	public static String COUNT_BY_I_S = PmlHoSoCongViecFinder.class.getName()
			+ ".countByI_S";
	...
	public int countByKeywords(long userId, String keywords)
		...
		return countByS_T_N_I_V_I_I_T_U_H(userId, soHieuHSCVs, tieuDes, null,
				null, vanDeLienQuans, 0, 0, tomTatNoiDungs, hoatDong,
				andOperator);
	}

	public int countByS_T_N_I_V_I_I_T_U_H(long userId, String soHieuHSCV,
		...
		return countByS_T_N_I_V_I_I_T_U_H(userId, new String[] { soHieuHSCV },
				new String[] { tieuDe }, ngayMoHSCV, idLinhVuc,
				new String[] { vanDeLienQuan }, idDoQuanTrong, idTrangThaiHSCV,
				new String[] { tomTatNoiDung }, hoatDong, andOperator);
	}

	private int countByS_T_N_I_V_I_I_T_U_H(long userId, String[] soHieuHSCVs,
			String[] tieuDes, Date ngayMoHSCV, String idLinhVuc,
		...
		try {
			...
			String sql = CustomSQLUtil.get(COUNT_BY_S_T_N_I_V_I_I_T_U_H);

			sql = CustomSQLUtil.replaceKeywords(sql, "so_hieu_hscv",
					StringPool.LIKE, false, soHieuHSCVs);
			sql = CustomSQLUtil.replaceKeywords(sql, "tieu_de",
					StringPool.LIKE, false, tieuDes);
			// Ngay mo
			if (ngayMoHSCV == null) {
				sql = StringUtil
						.replace(
			...
			// Linh vuc
			sql = CustomSQLUtil.replaceKeywords(sql, "id_linh_vuc",
					StringPool.EQUAL, false, new String[] { idLinhVuc });
			sql = CustomSQLUtil.replaceKeywords(sql, "van_de_lien_quan",
					StringPool.LIKE, false, vanDeLienQuans);
			// Do quan trong
			if (idDoQuanTrong <= 0) {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "tom_tat_noi_dung",
					StringPool.LIKE, false, tomTatNoiDungs);
	...
	public List<PmlHoSoCongViec> findByKeywords(long userId, String keywords,
		...
		return findByS_T_N_I_V_I_I_T_U_H(userId, soHieuHSCVs, tieuDes, null,
				null, vanDeLienQuans, 0, 0, tomTatNoiDungs, hoatDong,
				andOperator, start, end, obc);
	}

	public List<PmlHoSoCongViec> findByS_T_N_I_V_I_I_T_U_H(long userId,
			String soHieuHSCV, String tieuDe, Date ngayMoHSCV,
		...
		return findByS_T_N_I_V_I_I_T_U_H(userId, new String[] { soHieuHSCV },
				new String[] { tieuDe }, ngayMoHSCV, idLinhVuc,
				new String[] { vanDeLienQuan }, idDoQuanTrong, idTrangThaiHSCV,
				new String[] { tomTatNoiDung }, hoatDong, andOperator, start,
				end, obc);
	}

	private List<PmlHoSoCongViec> findByS_T_N_I_V_I_I_T_U_H(long userId,
			String[] soHieuHSCVs, String[] tieuDes, Date ngayMoHSCV,
			String idLinhVuc, String[] vanDeLienQuans, long idDoQuanTrong,
			long idTrangThaiHSCV, String[] tomTatNoiDungs, String hoatDong,
			boolean andOperator, int start, int end, OrderByComparator obc)
			throws SystemException {
		...
		try {
			...
			String sql = CustomSQLUtil.get(FIND_BY_S_T_N_I_V_I_I_T_U_H);
			...
			return (List<PmlHoSoCongViec>) QueryUtil.list(q, getDialect(),
					start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		} finally {
	...
	public int countByID_T_C(long[] idTinhChats) throws SystemException {
		...
		try {
			...
			if ((idTinhChats != null) && (idTinhChats.length > 0)) {
				...
				for (int i = 0; i < idTinhChats.length; i++) {
					...
					if ((i == idTinhChats.length - 1)
							&& (idTinhChats.length > 1)) {
						query.append(idTinhChats[i]);
					}
				}
		...
		try {
			...
			if ((idTrangThaiHSCVs != null) && (idTrangThaiHSCVs.length > 0)) {
				...
				for (int i = 0; i < idTrangThaiHSCVs.length; i++) {
					...
					if ((i == idTrangThaiHSCVs.length - 1)
							&& (idTrangThaiHSCVs.length > 1)) {
						query.append(idTrangThaiHSCVs[i]);
					}
				}
	...
	// Ham tim kiem khoa ngoai trong ban pml_ho_so_cong_viec
	public int countByID_D_Q_T(long[] idDoQuanTrongs) throws SystemException {
		...
		try {
			...
			if ((idDoQuanTrongs != null) && (idDoQuanTrongs.length > 0)) {
				...
				for (int i = 0; i < idDoQuanTrongs.length; i++) {
					...
					if ((i == idDoQuanTrongs.length - 1)
							&& (idDoQuanTrongs.length > 1)) {
						query.append(idDoQuanTrongs[i]);
					}
				}
