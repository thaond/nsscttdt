...
//				resultList = PmlEdmLevelSendLocalServiceUtil.getPmlEdmIssuingPlacesWithPmlEdmLevelSend(levelSendId);
	 ...
	 *
	 */
	public static String getNumberDocumentReceipt(int documentRecordTypeId, long documentTypeId, long userId, int documentRecordTypeIdHidden,  boolean documentRecordTypeCode, boolean documentTypeCode, String addText, boolean departmentCode, boolean useYear) {
		String numberResult = null;
		try {
			String nameDocumentRecordType = "";
			try {
				...
				nameDocumentRecordType = documentRecordType.getDocumentRecordTypeCode();
			
			} catch (Exception e) {
				System.out.println("ERROR: get PmlEdmDocumentRecordType from [ " + documentRecordTypeId + "]" + PCCCDocumentUtil.class+ e.getMessage());
			}
			...
			PmlEdmDocumentRecordTo pmlEdmDocumentRecordTo = PmlEdmDocumentRecordToLocalServiceUtil.getDocumentRecordToBy_YearInUse_AgencyId_DocumentRecordTypeId(String.valueOf(yearCurrent), department.getAgencyId(), documentRecordTypeId);
			...
			String resTemp = "";
			if (documentTypeCode) {
				if (!documentSymbol.equals("")) {
					resTemp += "/" + documentSymbol;
				}
			} 
			
			if (documentRecordTypeCode) {
				if (!nameDocumentRecordType.equals("")) {
					if (!resTemp.equals("")) {
						resTemp += "-" + nameDocumentRecordType;
					} else {
						resTemp += "/" + nameDocumentRecordType;
					}
				} 
			...
			if (departmentCode) {
				if (!resTemp.equals("")) {
					resTemp += "-" + department.getAbbreviateName();
				} else {
					resTemp += "/" + department.getAbbreviateName();
				}
			}
...
//				PmlEdmDocumentRecordTo pmlEdmDocumentRecordTo = PmlEdmDocumentRecordToLocalServiceUtil.getDocumentRecordToBy_YearInUse_AgencyId_DocumentRecordTypeId(yearCurrent+"", department.getAgencyId(), documentRecordTypeId);
	...
	public static boolean isBookDocumentRecord(String mainDepartmentId, long documentReceiptId) {
		try {
			...
			System.out.println("ERROR: in method isBookDocumentRecord "+PCCCDocumentUtil.class);
			System.out.println(e.getMessage());
			e.printStackTrace();
		}
		return false;
