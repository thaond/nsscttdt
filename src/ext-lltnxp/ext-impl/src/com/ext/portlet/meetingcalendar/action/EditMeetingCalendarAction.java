...
public class EditMeetingCalendarAction extends PortletAction {
	...
	public void processAction(
		...
		try {
			if (cmd.equals(Constants.ADD) || cmd.equals(Constants.UPDATE)) {
				...
				String fileName =
					ParamUtil.getString(actionRequest, "attachment");
	...
	protected void getFile(long mcalId, String fileName, HttpServletResponse res)
		throws Exception {

		InputStream is = null;

		try {
			MeetingCalendar mCal =
				MeetingCalendarLocalServiceUtil.getMCalById(mcalId);

			is =
				DLLocalServiceUtil.getFileAsStream(
					mCal.getCompanyId(), CompanyConstants.SYSTEM, "MCALFolder" +
						"/" + String.valueOf(mCal.getMcalId()) + "/" + fileName);
	...
	public ActionForward render(
		...
		return mapping.findForward(getForward(
			renderRequest, "portlet.ext.meeting_calendar.edit_event"));
	}
	...
	protected void deleteEvent(ActionRequest actionRequest)
		throws Exception {
		...
		MeetingCalendar calendar =
			MeetingCalendarLocalServiceUtil.getMeetingCalendar(eventId);
		calendar.setDeleted(true);
		MeetingCalendarLocalServiceUtil.updateMeetingCalendar(calendar);
	}

	protected void cancelEvent(ActionRequest actionRequest)
		throws Exception {
	...
	protected void updateEvent(
		...
		/*
		 * Duy Duong close start 21/12/2008 if(Validator.isNull(description)){
		...
		int durationMinute =
			ParamUtil.getInteger(actionRequest, "durationMinute");
		boolean allDay = ParamUtil.getBoolean(actionRequest, "allDay");
		boolean timeZoneSensitive =
			ParamUtil.getBoolean(actionRequest, "timeZoneSensitive");
		...
		if (repeating) {
			...
			if (recurrenceType == Recurrence.DAILY) {
				int dailyType =
					ParamUtil.getInteger(actionRequest, "dailyType");
				...
				if (monthlyType == 0) {
					int monthlyDay =
						ParamUtil.getInteger(actionRequest, "monthlyDay0");
				...
				int yearlyType =
					ParamUtil.getInteger(actionRequest, "yearlyType");

				if (yearlyType == 0) {
					int yearlyMonth =
						ParamUtil.getInteger(actionRequest, "yearlyMonth0");
					int yearlyDay =
						ParamUtil.getInteger(actionRequest, "yearlyDay0");
			...
			if (endDateType == 1) {
				int endDateOccurrence =
					ParamUtil.getInteger(actionRequest, "endDateOccurrence");
		...
		int firstReminder =
			ParamUtil.getInteger(actionRequest, "firstReminder");
		int secondReminder =
			ParamUtil.getInteger(actionRequest, "secondReminder");
		...
		ObjectValuePair<String, byte[]> file =
			new ObjectValuePair<String, byte[]>(null, null);
		if ((bytes != null) && (bytes.length > 0)) {
			file = new ObjectValuePair<String, byte[]>(fileName, bytes);
			/*
			 * ServiceContext serviceContext =
			 * ServiceContextFactory.getInstance( CalEvent.class.getName(),
			 * actionRequest);
			 */
		}
		...
		/*
		 * long[] listOrgIds = getLongArray( actionRequest,
		 * "organizationsSearchContainerPrimaryKeys");
		 */
		String[] orgIds =
			StringUtil.split(ParamUtil.getString(actionRequest, "orgIds"));
		String strorgIds = "";
		...
		String[] communityPermissions =
			actionRequest.getParameterValues("communityPermissions");
		String[] guestPermissions =
			actionRequest.getParameterValues("guestPermissions");
		...
		if (eventId <= 0) {
			...
			calEvent =
				MeetingCalendarServiceUtil.addEvent(
					...
					secondReminder, communityPermissions, guestPermissions,
					morning, afternoon, evening, prior);

			MeetingCalendarLocalServiceUtil.addMCal(
				place, place_diff, component, note, calEvent, repeatWeek, file,
				fileName, morning, afternoon, evening, prior);
			MeetingCalComponentLocalServiceUtil.addMCalComponent(
				calEvent.getEventId(), liststrorgIds, startDate.getTime());
			...
			if (startDate.getTime().equals(oldEvent.getStartDate())) {
				...
				MeetingCalendarServiceUtil.updateEvent(
					layout.getPlid(), eventId, title, description,
					startDateMonth, startDateDay, startDateYear, startDateHour,
					startDateMinute, endDateMonth, endDateDay, endDateYear,
					durationHour, durationMinute, allDay, timeZoneSensitive,
					type, repeating, recurrence, remindBy, firstReminder,
					secondReminder, morning, afternoon, evening, prior);

				/*
				 * CalEventLocalServiceUtil.updateEvent( layout.getPlid(),
				...
				MeetingCalendarLocalServiceUtil.updateMCal(
					eventId, place, place_diff, component, note, repeatWeek,
					file, morning, afternoon, evening, prior);
				MeetingCalComponentLocalServiceUtil.updateMCalComponent(
					eventId, liststrorgIds, startDate.getTime());
				...
				if (oldMCal.getState() == MeetingCalendarKey.DA_DUYET) {
					...
					calEvent =
						MeetingCalendarServiceUtil.addEvent(
							layout.getPlid(), title, description,
							startDateMonth, startDateDay, startDateYear,
							startDateHour, startDateMinute, endDateMonth,
							endDateDay, endDateYear, durationHour,
							durationMinute, allDay, timeZoneSensitive, type,
							repeating, recurrence, remindBy, firstReminder,
							secondReminder, communityPermissions,
							guestPermissions, morning, afternoon, evening,
							prior);
					MeetingCalendarLocalServiceUtil.addMCal(
						place, place_diff, component, note, calEvent,
						MeetingCalendarKey.DA_DUYET, repeatWeek, file,
						fileName, morning, afternoon, evening, prior);
					MeetingCalendarLocalServiceUtil.moveMCal(
						oldEvent.getEventId(), calEvent.getEventId());
					MeetingCalComponentLocalServiceUtil.addMCalComponent(
						calEvent.getEventId(), liststrorgIds,
						startDate.getTime());
					...
					if (oldMCal.getUserId() == user.getUserId()) {
						...
						MeetingCalendarServiceUtil.updateEvent(
							...
							secondReminder, morning, afternoon, evening, prior);
						MeetingCalendarLocalServiceUtil.updateMCal(
							eventId, place, place_diff, component, note,
							repeatWeek, file, morning, afternoon, evening,
							prior);
						MeetingCalComponentLocalServiceUtil.updateMCalComponent(
							eventId, liststrorgIds, startDate.getTime());
		...
		portletURL.setParameter("tabs1", "approved-calendar");

		actionResponse.sendRedirect(portletURL.toString());
	}

	protected void approveMCal(
		ActionRequest actionRequest, ActionResponse actionResponse)
		throws Exception {
	...
	/*
	 * Duy Duong close start 21/12/2008 protected void validate(String place,
	 * String place_diff) throws PortalException{ if((Validator.isNotNull(place)
	 * && Validator.isNotNull(place_diff))|| (Validator.isNull(place) &&
	 * Validator.isNull(place_diff)) ){ throw new DuplicateRoomException(); } }
	 */
	protected void validate(String place, String place_diff)
