...
<liferay-util:include page="/html/portlet/ext/delegate/js/validateForm.jsp"></liferay-util:include>
...
<portlet:actionURL var="add">
	...
	<portlet:param name="struts_action" value="/sgs/delegate/view"/>
</portlet:actionURL>
<form action="<%= add %>" method="post" name="<portlet:namespace />fm" onsubmit="return validate();">
	...
	%>
	<div class="title_categ"><liferay-ui:message key="javax.portlet.title.SGS_DELEGATE"/></div>
				...
				<td><label><liferay-ui:message key="delegate-nguoiduocuyquyen"/>&nbsp;:<font color="red">(*)</font></label></td>
					<td>
						<select style="width: 51%" name="<portlet:namespace />department" onchange="<portlet:namespace />changeDepartment();">
							...
							<%
							// Doc bang pml_department de lay ra danh sach
							List<Department> deps = DepartmentUtil.findAll();
						...
						<select style="width: 30%" name="receiver" id="receiver">
							<option value="" selected="selected">[-- <liferay-ui:message key="select"/> --]</option>
							<%-- xuancong close
					<logic:iterate id="receiver" name="receivers" indexId="i" type="com.liferay.portal.model.User" scope="request" >
					<%
						String receiverName = receiver.getLastName() + " " + receiver.getMiddleName() + " " + receiver.getFirstName();
					%>
					<option value="<%=receiver.getPrimaryKey() %>"><%=receiverName %></option>
					</logic:iterate>
							--%>
				</select>
				</td>
			...
			<tr>
				<td valign="top"><label><liferay-ui:message key="delegate-loaihoso"/>&nbsp;:<font color="red">(*)</font> </label></td>
					...
					<tr class="portlet-section-header results-header" >
							...
							<logic:iterate id="fileType" name="fileTypes" indexId="k" type="com.sgs.portlet.filetype.model.PmlFileType" scope="request" >						
								<%
									String check = "";
...
<script language="javascript">
	// Mac dinh chon tat ca nguoi dung
	<portlet:namespace />changeDepartment("");
</script>