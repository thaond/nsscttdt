...
List<CarResource> carResources = new ArrayList<CarResource>();
try {
...
try {
	carResources = CarResourceServiceUtil.getCarResources();
} catch (Exception e) {
...
<form action="<portlet:actionURL windowState="<%= WindowState.MAXIMIZED.toString() %>"><portlet:param name="struts_action" value="/ext/meeting_calendar/edit_event" /></portlet:actionURL>" method="post" enctype="multipart/form-data" name="<portlet:namespace />fm" onSubmit="<portlet:namespace />saveEvent(); return false;" class="meeting_calendar_bg">
...
<table cellspacing="0" width="100%">
		...
		<tr>
			...
			<td class="td-right">
				<input name="<portlet:namespace />title" id="<portlet:namespace />title" type="text" size=20 value="<%= title_cal %>" style="width:500px" />
			</td>
		</tr>
		<tr>
			<td class="td-left">
		...
		<tr>
			<td class="td-left">
				<liferay-ui:message key="room-meeting" />
			</td>
			<td class="td-right">
				<%
				...
				<select name="<portlet:namespace />place">
					...
					<%for (RoomResource room: roomResources) {
						%>
						<option value="<%=room.getRoomId() %>" <%= (mcalendar != null) && (mcalendar.getPlace().equals(String.valueOf(room.getRoomId()))) ? "selected":"" %>><%=HtmlUtil.escape(room.getRoomName()) %></option>
						<%
					}%>
				</select>
		...
		<tr>
			<td class="td-left">
				<liferay-ui:message key="car-resource" /> :
			</td>
			<td class="td-right">
				<select name="<portlet:namespace/>note">
					<option value="0">--<liferay-ui:message key="select-car" />--</option>
					<%for (CarResource car : carResources) {
						%>
						<option value="<%=car.getCarId() %>" <%= (mcalendar != null) && (mcalendar.getNote().equals(String.valueOf(car.getCarId()))) ? "selected":"" %>><%=HtmlUtil.escape(car.getCarCode()) %></option>
						<%
					}%>
				</select>
			</td>
	...
	<td valign="top">
		...
		List parentresults = OrganizationLocalServiceUtil.search(company.getCompanyId(), parentOrganizationId, null, -1, null, null, null, null, null, organizationParams, true, -1, -1, searchContainerOrg.getOrderByComparator());
		%>
		...
		for (int i = 0; i < parentresults.size(); i++) {
			...
			List results = OrganizationLocalServiceUtil.search(company.getCompanyId(), parentcurOrganization.getOrganizationId(), null, -1, null, null, null, null, null, organizationParams, true, -1, -1, searchContainerOrg.getOrderByComparator());
...
<script type="text/javascript">
	...
	function <portlet:namespace />saveFileCalendar() {
		...
		if(actionId == "" || actionId == "0"){
			var url = '<liferay-portlet:actionURL windowState="<%=WindowState.MAXIMIZED.toString()%>"><portlet:param name="struts_action" value="/ext/iereport/addnew" /></liferay-portlet:actionURL>';
			var checkValue = document.getElementById("ckbActive");
			...
			var url = '<liferay-portlet:actionURL windowState="<%=WindowState.MAXIMIZED.toString()%>"><portlet:param name="struts_action" value="/ext/iereport/updateaction" /></liferay-portlet:actionURL>';
			if(actionId != "" && actionId != "0"){
				if(checkValue.checked == true){
					document.<portlet:namespace />fm.<portlet:namespace />forwardCheckValue.value = 1;
				}
	...
	// Canh
	$jq(function(){
		pmlUserUtilClient.getSponsorUsers(function(data){
			var input = $jq("#<portlet:namespace />title");
			input.autocomplete({
				source: data,
				minLength: 0
			}).css('height', 20);
			$jq( "<button type='button'>&nbsp;</button>" )
			.attr( "tabIndex", -1 )
