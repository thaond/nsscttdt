...
import com.nss.portlet.onedoorprocess.searchresult.PmlOneDoorReceiveFileSearchResult;
...
public class SearchResultAction extends OneDoorProcessPortletAction {
	...
	public ActionForward renderStruts(ActionMapping mapping, ActionForm form,
		...
		return mapping
				.findForward("portlet.nss.onedoorprocess.searchresult");
	}
	...
	private List<PmlFilePcccDTO> getPmlFilePcccDTO(
		...
		for (PmlOneDoorReceiveFile file : list) {
			...
			try {
				fileType = PmlFileTypeLocalServiceUtil.getPmlFileType(file
						.getFileTypeId());
				filePcccDTO.setFiletype(fileType.getFileTypeName());
			} catch (Exception e) {
				filePcccDTO.setFiletype("");
			...
			if (log != null) {
				...
				try {
					PmlFileStatusStateProcess fileStatusStateProcess = PmlFileStatusStateProcessLocalServiceUtil
	...
	private List<PmlOneDoorReceiveFile> filterPmlOneDoorReceiveFiles(
		...
		portletURL.setParameter("struts_action",
				"/nss/onedoorprocess/searchresult");
