...
<%@page import="com.nss.portlet.position.search.PositionSearchTerms"%>
<%@page import="com.nss.portlet.position.model.Position"%>
<%@page import="com.nss.portlet.position.service.PositionLocalServiceUtil"%>
<%@page import="com.nss.portlet.position.PositionCanNotDeleteException"%>
...
<form action="<%= portletURL.toString() %>" method="post" name="<portlet:namespace />fm">
	...
	<liferay-ui:error exception="<%= PositionCanNotDeleteException.class %>" message="can-not-delete-field" />
	<div class="parent-title"><liferay-ui:message key="position"/></div>
	<%
	...
	%>
		...
		<liferay-ui:search-form	page="/html/portlet/nss/position/search_form.jsp" searchContainer="<%= positionSearch %>" />	
		<portlet:renderURL var="add">
			<portlet:param name="struts_action" value="/nss/position/view" />
	...
	<%
		...
		for (int i = 0; i < results.size(); i ++) {
			...
			rowURLActive.setParameter("struts_action", "/nss/position/view");
			...
			rowURLEdit.setParameter("struts_action", "/nss/position/view");
			...
			rowURLDelete.setParameter("struts_action", "/nss/position/view");
