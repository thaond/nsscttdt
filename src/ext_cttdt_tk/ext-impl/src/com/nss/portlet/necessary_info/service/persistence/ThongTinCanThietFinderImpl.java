...
public class ThongTinCanThietFinderImpl extends BasePersistenceImpl implements ThongTinCanThietFinder{
	public static String FIND_BY_TEN_MOTA = ThongTinCanThietFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = ThongTinCanThietFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(tenthongtincanthiet)", StringPool.LIKE, false, names);
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(motathongtincanthiet)", StringPool.LIKE, true, des);
	...
	public List<ThongTinCanThiet> findTEN_MOTA(String name, String des, boolean andOperator, int start, int end, OrderByComparator obc) throws SystemException {
		return findTEN_MOTA(new String[]{name}, new String[]{des}, andOperator, start, end, obc);
	}

	public List<ThongTinCanThiet> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_thong_tin_can_thiet", ThongTinCanThietImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<com.nss.portlet.necessary_info.model.ThongTinCanThiet>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
