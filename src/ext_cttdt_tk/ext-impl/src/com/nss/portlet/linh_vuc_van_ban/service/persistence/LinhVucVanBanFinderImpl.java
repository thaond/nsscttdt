...
public class LinhVucVanBanFinderImpl extends BasePersistenceImpl implements LinhVucVanBanFinder{
	public static String FIND_BY_TEN_MOTA = LinhVucVanBanFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = LinhVucVanBanFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_linh_vuc_van_ban)", StringPool.LIKE, false, names);
	...
	public List<LinhVucVanBan> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_linh_vuc_van_ban", LinhVucVanBanImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<LinhVucVanBan>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
