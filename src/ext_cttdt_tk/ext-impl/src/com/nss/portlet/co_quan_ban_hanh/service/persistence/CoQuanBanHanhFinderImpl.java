...
public class CoQuanBanHanhFinderImpl extends BasePersistenceImpl implements CoQuanBanHanhFinder {
	public static String FIND_BY_TEN_MOTA = CoQuanBanHanhFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = CoQuanBanHanhFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_co_quan_ban_hanh)", StringPool.LIKE, false, names);
	...
	public List<CoQuanBanHanh> findTEN_MOTA(String name, String des, boolean andOperator, int start, int end, OrderByComparator obc) throws SystemException {
		return findTEN_MOTA(new String[]{name}, new String[]{des}, andOperator, start, end, obc);
	}

	public List<CoQuanBanHanh> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_co_quan_ban_hanh", CoQuanBanHanhImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<CoQuanBanHanh>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
