...
public class DonViThuTucFinderImpl extends BasePersistenceImpl implements DonViThuTucFinder {
	
	public static String FIND_BY_TEN_MOTA = DonViThuTucFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = DonViThuTucFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_don_vi_thu_tuc)", StringPool.LIKE, false, names);
	...
	public List<DonViThuTuc> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_don_vi_thu_tuc", DonViThuTucImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<DonViThuTuc>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
