...
public class QuangCaoFinderImpl extends BasePersistenceImpl implements QuangCaoFinder{
	public static String FIND_BY_TEN_MOTA = QuangCaoFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = QuangCaoFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(tenquangcao)", StringPool.LIKE, false, names);
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(motaquangcao)", StringPool.LIKE, true, des);
	...
	public List<QuangCao> findTEN_MOTA(String name, String des, boolean andOperator, int start, int end, OrderByComparator obc) throws SystemException {
		return findTEN_MOTA(new String[]{name}, new String[]{des}, andOperator, start, end, obc);
	}

	public List<QuangCao> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_quang_cao", QuangCaoImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<QuangCao>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
