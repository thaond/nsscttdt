...
public class LienKetWebsiteFinderImpl extends BasePersistenceImpl implements LienKetWebsiteFinder{
	public static String FIND_BY_TEN_MOTA = LienKetWebsiteFinder.class.getName() + ".findBy_TEN_MOTA";
	public static String COUNT_BY_TEN_MOTA = LienKetWebsiteFinder.class.getName() + ".countBy_TEN_MOTA";
	...
	public int countByTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(ten_website)", StringPool.LIKE, false, names);
			sql = CustomSQLUtil.replaceKeywords(sql, "lower(mo_ta_website)", StringPool.LIKE, true, des);
	...
	public List<LienKetWebsite> findTEN_MOTA(String[] names, String[] des,
		...
		try {
			...
			q.addEntity("nss_lien_ket_website", LienKetWebsiteImpl.class);
			
			QueryPos qpos = QueryPos.getInstance(q);
			qpos.add(names,2);
			qpos.add(des,2);
			 
			return (List<LienKetWebsite>) QueryUtil.list(q, getDialect(), start, end);
		} catch (Exception e) {
			throw new SystemException(e);
		}
		finally{
