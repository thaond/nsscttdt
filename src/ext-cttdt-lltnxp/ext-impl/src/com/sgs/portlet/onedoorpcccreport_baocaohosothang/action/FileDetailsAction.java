...
import com.sgs.portlet.onedoorpcccreport_baocaohosothang.searchfiledetail.PmlOneDoorReceiveFileSearch;
...
public class FileDetailsAction extends PortletAction {
	...
	public ActionForward render(ActionMapping mapping, ActionForm form,
			...
			if (getForward(req) != null && !getForward(req).equals("")) {
				...
				return mapping.findForward("portlet.sgs.onedoorpcccreport_baocaohosothang.filedetails");
			}
	}
	...
	private List<PmlFilePcccDTO> getListPmlFilePcccDTO(List<PmlOneDoorReceiveFile> pmlOneDoorReceiveFileList){
		...
		for (PmlOneDoorReceiveFile pmlOneDoorReceiveFile : pmlOneDoorReceiveFileList) { 
			...
			if (log != null) {
				...
				try {
					logList = PmlProcessDisposeUtil.findByFileId(fileId);
				...
				if ( logList.size() != 0) {
					try {
						log = PmlProcessDisposeUtil.findByFileId_Transition(fileId, logList.size()).get(0);
