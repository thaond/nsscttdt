...
//			IdTemplateServiceUtil.generatedStringId(PmlEdmDocumentSend.class.getName());
		...
		//xuancong close String abbreviateName = department.getAbbreviateName();
		//xuancong close String departmentCode = department.getDepartmentsCode();
		String departmentsId = department.getDepartmentsId();
		//neu la loai cong van
		
		
		//xuancong close int documentTypeId = (int) document.getDocumentTypeId();
		long documentTypeId = document.getDocumentTypeId();
		PmlEdmDocumentType edmDocumentType = PmlEdmDocumentTypeUtil.findByPrimaryKey(documentTypeId);
		int documentRecordtypeId = edmDocumentType.getDocumentRecordTypeId(); 
		PmlEdmDocumentRecordType documentType = PmlEdmDocumentRecordTypeUtil.findByPrimaryKey(documentRecordtypeId);
		
		String documentRecordTypeCode = documentType.getDocumentRecordTypeCode();
		//xuancong close String documentsymbol = edmDocumentType.getDocumentSymbol();
		//xuancong close String typeDocumentSend =edmDocumentType.getDocumentTypeName();
		//xuancong close String loaivanbanCVD = PropsUtil.get("type.document.send");
		//xuancong close String abbreviationAgencyName = PropsUtil.get("abbreviation.agency.name");
		//xuancong close String documentTypeName = ParamUtil.getString(req, "loaicongvancvdtn");
		
		/* xuancong close start
		if (edmDocumentType.getDocumentSymbol().equals(loaivanbanCVD)) {
			int documentTypeIdCount = PmlEdmDocumentSendLocalServiceUtil.countDocumentType(documentTypeId,abbreviateName);
			int documentTypeIdCountIncrement = documentTypeIdCount+1;
			
			document.setDocumentReference(String.valueOf(documentTypeIdCountIncrement+"/"+abbreviationAgencyName.replaceFirst("-", "")+abbreviateName));
			
			//DocumentSendDTO documentSendDTO = populateDocumentSendToDTO(document);
		}
		else{
		//set so cv di vua phat sinh vao
		int documentTypeIdCount = PmlEdmDocumentSendLocalServiceUtil.countDocumentType(documentTypeId,abbreviateName);
		int documentTypeIdCountIncrement = documentTypeIdCount+1;
		
		document.setDocumentReference(String.valueOf(documentTypeIdCountIncrement+"/"+ documentsymbol +abbreviationAgencyName+ abbreviateName));
		
		//DocumentSendDTO documentSendDTO = populateDocumentSendToDTO(document);
		}
		xuancong close end */
		// xuancong add -- chinh lai cach phat sinh so van ban di
		document.setDocumentReference(PCCCDocumentUtil.getNumberDocumentPublishByTDY(documentTypeId, departmentsId, 0));
		...
		return mapping
				.findForward("portlet.sgs.pcccdocumentsendprocess.saveAndPublish");
	}
	...
	private void updateDocumentSend(ActionRequest req, ActionResponse res) throws Exception{
		...
		String phanMoRong = ParamUtil.getString(req, "phanMoRong"); // phmphuc 01/12/2010 - gan phan mo rong phong ban
...
//		PmlEdmDocumentSendLocalServiceUtil.updatePmlEdmDocumentSend(document);
		...
		PmlEdmDocumentSendLocalServiceUtil.updatePmlEdmDocumentSend(user.getCompanyId(), document);
		//end of Canh
