	...
	public void signalBackDocumentReceipt(List<WorkflowParam> params) {
		...
		try {
			...
			if (Boolean.parseBoolean(rutLai)) { // start yenlt 20101119
				...
				try {
					pMSList = PmlMainSupportUtil.findByprocessIdMain_Type(processId, TYPE_BC);
				} catch (Exception e) {
					log.error("ERROR: get list PmlMainSupport from processInstanceId=["+processId+"] and type=["+TYPE_BC+"] "+ e.getMessage());
				}
				...
				for (int i = pMSListSize-1; i >= 0; i--) {
					if (getJbpmService().getProcessInstance(pMSList.get(i).getProcessId()).getEnd() == null) {
						processInstanceReportSupprotFinalId = pMSList.get(i).getProcessId();
				...
				if (isProcessInstanceReport) {
					...
					List<PmlDocumentReceiptLog> documentReceiptLogSupportFinals = new ArrayList<PmlDocumentReceiptLog>();
					try {
						documentReceiptLogSupportFinals = PmlDocumentReceiptLogUtil.findByProcessInstanceId_Type(processInstanceReportSupprotFinalId, 4);
						
						Collections.sort(documentReceiptLogSupportFinals, new Comparator<PmlDocumentReceiptLog>() {
							public int compare(PmlDocumentReceiptLog o1, PmlDocumentReceiptLog o2) {
								return (o1.getTransition_()-o2.getTransition_());
							}
							
						});
						documentReceiptLogSupportFinal = documentReceiptLogSupportFinals.get(0);
					} catch (Exception e) {}
					
					List<PmlDocumentReceiptLog> documentReceiptLogSuppors = new ArrayList<PmlDocumentReceiptLog>();
					try {
						documentReceiptLogSuppors = PmlDocumentReceiptLogUtil.findByDocumentReceiptId_Type(documentReceiptId, 4);
					} catch (Exception e) {	}
					
					if (documentReceiptLogSuppors == null) {
						documentReceiptLogSuppors = new ArrayList<PmlDocumentReceiptLog>();
					}
					...
					for (PmlDocumentReceiptLog pmlDoLog : documentReceiptLogSuppors) {
						if (documentReceiptLogSupportFinal != null) {
							if (pmlDoLog.getDateProcessTime() != null && documentReceiptLogSupportFinal.getDateProcessTime().getTime() == pmlDoLog.getDateProcessTime().getTime()) {
								listTaskInstance = getJbpmService().getListTasksNode(pmlDoLog.getProcessInstanceId());
								for (int j = 0; j < listTaskInstance.size(); j++) {
									TaskInstance taskInstance = listTaskInstance.get(j);
									if (taskInstance != null && taskInstance.getEnd() == null) {
										String taskTransition =  taskInstance.getName().split(":")[2];
										getJbpmService().signalTask(taskInstance.getId(), taskTransition);
										listTaskInstance = getJbpmService().getListTasksNode(pmlDoLog.getProcessInstanceId());
									}
								}
								
								for (PmlDocumentReceiptLog pmlDoLog1 : documentReceiptLogSuppors) {
									if ((pmlDoLog1.getProcessInstanceId() == pmlDoLog.getProcessInstanceId()) && pmlDoLog1.getDateProcessTime() == null) {
										stateProcessName = getStateNode(pmlDoLog1.getProcessInstanceId());
										stateProcessHoanThanh = getStateProcessFromCode(stateProcessName);
...
//							getJbpmService().deleteProcessInstance(processId);
	...
	public void signalReport(List<WorkflowParam> params) {
		long taskId = WorkflowParamUtil.getLong(params, "taskId", -1);
		String transition = WorkflowParamUtil.getString(params, "transition", "NotAvailable");
		String processInformation = WorkflowParamUtil.getString(params, "processInformation", "");
		String dateNumberProcess = WorkflowParamUtil.getString(params, "dateNumberProcess", "0");
		
		String receiverIdReportMain = WorkflowParamUtil.getString(params, "receiverIdReportMain", "");
		String receiverIdReports = WorkflowParamUtil.getString(params, "receiverIdReports", "");
		...
		if (taskId == -1 || "NotAvailable".equalsIgnoreCase(transition)
				|| "".equalsIgnoreCase(receiverIdReportMain) || "".equals(receiverIdReports)) {			
			log.info("ERROR: param invalidate");
			return;
		}
		...
		try {  
				long processInstanceId = getJbpmService().getProcessInstanceFromTaskInstance(taskId).getId();
				
				String [] receiverIdReportsArr = receiverIdReports.split(",");
				
				PmlDocumentReceiptWF pmlDocumentReceiptWF = PmlDocumentReceiptWFUtil.findByPrimaryKey(processInstanceId);
				long documentReceiptId = pmlDocumentReceiptWF.getDocumentReceiptId();
				...
				PmlDocumentReceiptLog pmlDocumentReceiptLog = PmlDocumentReceiptLogUtil.findByProcessInstanceId_StateProcessIdBefore_DateProcess(processInstanceId, stateProcess.getStateProcessId(), null).get(0);
				...
				for (int i = 0; i < receiverIdReportsArr.length; i++) {
					...
					// create instance of support process, signal process support to next node
					processInstanceSupport = getJbpmService().createProcessInstance(PROCESS_NAME_BC_SUPPORT);
					processInstanceSupportId = processInstanceSupport.getId();
					getJbpmService().setContext(processInstanceSupportId, "actorId", receiverIdReportsArr[i]);
					getJbpmService().signalProcessInstance(processInstanceSupportId);
					...
					PmlUser pmlUserNhan = null;
					try {
						pmlUserNhan = PmlUserLocalServiceUtil.getPmlUser(Long.parseLong(receiverIdReportsArr[i]));
					} catch (Exception e) {
					}
					if (pmlUserNhan != null) {
						pmlDocumentReceiptLog.setDepartmentReceiveId(pmlUserNhan.getDepartmentsId());
					...
					try {
						transitionHelp = PmlDocumentReceiptLogUtil.countByDocumentReceiptId(documentReceiptId) + 1;
					} catch (Exception e) {}
					
					pmlDocumentReceiptLog.setTransition_(transitionHelp);
					
					pmlDocumentReceiptLog = PmlDocumentReceiptLogLocalServiceUtil.addPmlDocumentReceiptLog(pmlDocumentReceiptLog);
				
					
					documentReceiptLogSupport = logNewNode(documentReceiptId, receiverIdReportsArr[i], processInstanceSupportId, new Date(), 4, pmlDocumentReceiptLog.getStep() + 1);
					
					pmlDocumentReceiptLog.setStateProcessIdAfter(documentReceiptLogSupport.getStateProcessIdBefore());
					PmlDocumentReceiptLogLocalServiceUtil.updatePmlDocumentReceiptLog(pmlDocumentReceiptLog);
					
					logNewMessage(documentReceiptId, processor, Long.parseLong(receiverIdReportsArr[i]));
				}
				...
				// signal process main to next node, set actorId and processInformation to process instance. Then, end task
				getJbpmService().setContext(processInstanceId, "actorId", receiverIdReportMain);
				
				// load current Log information
				pmlDocumentReceiptLog = PmlDocumentReceiptLogUtil.findByProcessInstanceId_StateProcessIdBefore_DateProcess(processInstanceId, stateProcess.getStateProcessId(), null).get(0);
				pmlDocumentReceiptLog.setProcessInfomation(processInformation);
				pmlDocumentReceiptLog.setNumDateProcess(Integer.parseInt(dateNumberProcess));
				...
				// set StateProcessIdAfter for current log.
				pmlDocumentReceiptLog.setStateProcessIdAfter(getStateProcessFromCode(getStateNode(processInstanceId)).getStateProcessId());
				...
				PmlMainSupport pmlMainSupportTemp = null;
				try {
					pmlMainSupportTemp  = PmlMainSupportUtil.findByPrimaryKey(processInstanceId);
				} catch (Exception e) {
				...
				if (pmlMainSupportTemp == null) {
					try {
						...
						pmlEdmDocumentReceipt.setStatusId(getStateProcessFromCode(getStateNode(processInstanceId)).getStateProcessId());
						PmlEdmDocumentReceiptLocalServiceUtil.updatePmlEdmDocumentReceipt(pmlEdmDocumentReceipt);
					} catch (Exception e) {
					}
				}
				...
				if (getJbpmService().getProcessInstance(processInstanceId).getEnd() == null) {
					logNewNode(documentReceiptId, receiverIdReportMain, processInstanceId, dateProcess, pmlDocumentReceiptLog.getType_(), pmlDocumentReceiptLog.getStep() + 1);
				}
			...
			log.error("ERROR: in method signalReport of class " + DocumentReceiptLiferayWorkflowService.class);
			e.printStackTrace();
		}
	}
}
