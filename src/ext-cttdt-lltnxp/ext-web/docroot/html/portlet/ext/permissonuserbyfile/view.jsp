...
<%@page import="com.sgs.portlet.permissonuserbyfile.search.PermissionUserByFileSearch"%>
...
<%@page import="com.sgs.portlet.country.pmluserfiletype.service.persistence.PmlUserFileTypeUtil"%>
...
<%
	...
	portletURL.setParameter("struts_action", "/sgs/permissonuserbyfile/view");
	
%>

<portlet:actionURL var="form" windowState="<%= WindowState.NORMAL.toString() %>">
	<portlet:param name='struts_action' value='/sgs/permissonuserbyfile/view'/>
</portlet:actionURL>

<form action="<%= form %>" method="post" name="<portlet:namespace />fm">
<input type="hidden" name="<portlet:namespace />multiUsers">
<div class="title_categ"><liferay-ui:message key="javax.portlet.title.SGS_PERMISSONUSERBYFILE"/></div>
<div class="boxcontent">
	...
	<%
		...
		for (int i = 0; i < results.size(); i++) {
			...
			try {
				pmlUserFileTypeList = PmlUserFileTypeUtil.findByUserId(userId);
			} catch (Exception e) {
			...
			phanQuyenURL.setParameter("struts_action","/sgs/permissonuserbyfile/edit");
