...
<%
	...
	portletURL.setParameter("struts_action", "/sgs/pcccdocumentsendprocess/search");
...
<liferay-ui:search-form	
	page="/html/portlet/ext/pcccdocumentsendprocess/timcongvanden_filter.jsp"
	searchContainer="<%= searchContainer %>" ></liferay-ui:search-form>

<%
try {
	if (Boolean.valueOf(renderRequest.getAttribute("receiptFlag").toString()) == true) {
		...
		// end of canh
		//thoi start
		// Trong trang tim kiem nay ko dua vao chuc nang xoa, cot thu 11
		searchContainer.getHeaderNames().remove(10);
		if (!PortletPermissionUtil.contains(permissionChecker, plid, "SGS_PCCCDOCUMENTSENDPROCESS", ActionKeysExt.TIEPNHANCV)) {
			//xuancong close, cot edit chuyen tu 2 sang 10 searchContainer.getHeaderNames().remove(1);
			searchContainer.getHeaderNames().remove(9);
	...
	<%
		...
		for (int i = 0; i < hits.getDocs().length; i ++) {
			...
			if (null != pmlEdmDocumentReceipt) { // end yenlt 31052010
				...
				// Loai cong van
				// minh update 20091215 [ new loai cong van khong ton tai trong db ]
				...
				if (issuingPlaceId.equals("")) {
					...
					// minh update 20091215 [ neu noi phat hanh khong ton tai trong db ]
				...
				row.addText(new SimpleDateFormat("dd/MM/yyyy").format(pmlEdmDocumentReceipt.getIssuingDate()));
				...
				for (int j = 0; j < attachedFiles.size(); j ++) {
					...
					//xuancong close toanVanURL += "<a href=\"" + attachedFile.getPath() + "\"> <img src='/html/images/icon_download.png'>&nbsp;" + (!attachedFile.getTitle().equals("") ? attachedFile.getTitle() : attachedFile.getAttachedFileName()) + "</a><br />";
