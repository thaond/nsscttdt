...
<table cellspacing="0" width="100%">
	...
	<tr>
		...
		<td width="35%"><input style="width: 90%;" type="text" name="<portlet:namespace /><%= displayTerms.SO_PHAT_HANH %>" value="<%= HtmlUtil.escape(displayTerms.getSoPhatHanh()) %>" id="textfield"></td>
		...
		<td>
    		...
    		<option <%=(displayTerms.getLoaiCongVan() == 0 ? "selected" : "" )%> value="0"><liferay-ui:message key="documentreceipt-search-tatca" /></option>
				<logic:iterate id="pmlEdmDocumentType" name="pmlEdmDocumentTypeList" indexId="i" type="com.sgs.portlet.document.receipt.model.PmlEdmDocumentType">
					<%-- xuancong close fix loi khong lay lai duoc loai van ban    			
					<option <%=(displayTerms.getLoaiCongVan() == pmlEdmDocumentType.getDocumentRecordTypeId() ? "selected" : "" )%> value="<%= pmlEdmDocumentType.getDocumentTypeId() %>"><%= pmlEdmDocumentType.getDocumentTypeName() %></option>
					--%>
					<option <%=(displayTerms.getLoaiCongVan() == pmlEdmDocumentType.getDocumentTypeId() ? "selected" : "" )%> value="<%= pmlEdmDocumentType.getDocumentTypeId() %>"><%= pmlEdmDocumentType.getDocumentTypeName() %></option>
      		</logic:iterate>
    		</select>
		</td>
	</tr>
	<tr>
		...
		<td>
			...
			<option <%=(displayTerms.getPhongSoanThao().equals("") ? "selected" : "" )%> value=""><liferay-ui:message key="documentreceipt-search-tatca" /></option>
				<logic:iterate id="pmlDepartment" name="pmlDepartmentList" indexId="i" type="com.sgs.portlet.department.model.Department"  >				
		...
		<td>
			<input type="text" name="<portlet:namespace /><%= displayTerms.NOI_NHAN %>" value="<%= HtmlUtil.escape(displayTerms.getNoiNhan()) %>" style="width: 90%;">
	...
	<tr>
		...
		<td>
			<select style="width: 93%;" name="<portlet:namespace /><%= displayTerms.SO_HSCV %>" id="select">
	...
	<tr>
		...
		<td colspan="3"><textarea style="width: 95%;" name="<portlet:namespace /><%= displayTerms.TRICH_YEU %>"  id="textarea" rows="2"><%= HtmlUtil.escape(displayTerms.getTrichYeu()) %></textarea></td>
	</tr>
	<tr>
		<td></td>
		<td colspan="3">
