	...
	<div class="search-results">
		...
		for (int i = pagesIteratorBegin; i <= pagesIteratorEnd; i++) {
			if (i == curValue) {
				...
				sb.append("<a class='journal-article-page-number' onchange='viewListDocumentReport(");
...
<c:if test="<%= total > delta %>">
	<div class="search-pages">
			...
			<div class="page-selector">
				...
				<c:choose>
					...
					<c:otherwise>
						...
						<select style="width: 70px;" onchange='viewListDocumentReport("<%= url + curParam %>" + "=" + this.value + "<%= urlAnchor %>")'>
		...
		<div class="page-links">
			<c:if test='<%= type.equals("regular") %>'>
				<c:choose>
					<c:when test="<%= curValue != 1 %>">
						<a class="first" onclick="viewListDocumentReport('<%= _getHREF(formName, curParam, 1, jsCall, url, urlAnchor) %>')" target="<%= target %>">
					</c:when>
					<c:otherwise>
						<span class="first">
					</c:otherwise>
			...
			<c:choose>
				<c:when test="<%= curValue != 1 %>">
					<a class="previous" onclick="viewListDocumentReport('<%= _getHREF(formName, curParam, curValue - 1, jsCall, url, urlAnchor) %>')" target="<%= target %>">
				</c:when>
			...
			<c:choose>
				<c:when test="<%= curValue != pages %>">
					<a class="next" onclick="viewListDocumentReport('<%= _getHREF(formName, curParam, curValue + 1, jsCall, url, urlAnchor) %>')" target="<%= target %>">
				</c:when>
				<c:when test='<%= type.equals("regular") %>'>
					<span class="next">
				</c:when>
			...
			<c:if test='<%= type.equals("regular") %>'>
				<c:choose>
					<c:when test="<%= curValue != pages %>">
						<a class="last" onclick="viewListDocumentReport('<%= _getHREF(formName, curParam, pages, jsCall, url, urlAnchor) %>')" target="<%= target %>">
					</c:when>
					<c:otherwise>
						<span class="last">
					</c:otherwise>
